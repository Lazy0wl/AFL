...
static const u8 *trampoline_fmt_32 =
    ...
    "movl %%eax, 12(%%esp)\n"
    "movl $0x%08x, %%ecx\n"  //把参数给ecx
    "call __afl_maybe_log\n" //调用__afl_maybe_log--> __afl_maybe_log(random() % MAP_SIZE) 来确定唯一的运行位置
    "movl 12(%%esp), %%eax\n"
    "movl  8(%%esp), %%ecx\n"
    "movl  4(%%esp), %%edx\n"
    "movl  0(%%esp), %%edi\n"
...
static const u8 *main_payload_32 =
    ...
    "  movl  __afl_area_ptr, %edx\n"                   //__afl_area_ptr存储的是共享内存的起始位置
    "  testl %edx, %edx\n"
    "  je    __afl_setup\n" //先判断server中是否已经设置共享内存，若没有先通过ID获取并设置
    "\n"
    "__afl_store:\n"
    "\n"
    ...
    "  movl __afl_prev_loc, %edi\n" //初始位置0 , 变量__afl_prev_loc保存的是前一次跳转的”位置”
    "  xorl %ecx, %edi\n"           // 异或后保存在edi中
    "  shrl $1, %ecx\n"             // 以edx（共享内存）为基址
    "  movl %ecx, __afl_prev_loc\n" // ecx的值右移1位后，保存在了变量__afl_prev_loc中
#else
    "  movl %ecx, %edi\n"
#endif /* ^!COVERAGE_ONLY */
    "\n"
    ...
    "__afl_setup:\n" //此处为初始共享内存，即通过shm_id得到共享内存并将其首地址进行放置
    "\n"
    "  /* Do not retry setup if we had previous failures. */\n"
    "\n"
    "  cmpb $0, __afl_setup_failure\n"
    "  jne  __afl_return\n" //当上次已经设置并且失败了，就不必再次尝试而是直接退出了
    "\n"
    ...
    "  pushl %eax        /* SHM ID         */\n"
    // 通过调用shmat()，target将这块共享内存也映射到了自己的内存空间中，并将其地址保存在__afl_area_ptr及edx中。由此，便完成了fuzzer与target之间共享内存的设置
    "  call  shmat\n" //获取共享内存地址
    "  addl  $12, %esp\n"
    "\n"
    "  cmpl $-1, %eax\n"
    ...
    "\n"
    "  pushl $4          /* length    */\n"
    "  pushl $__afl_temp /* data      */\n" //__afl_temp为4字节，即buffer
    "  pushl $" STRINGIFY((FORKSRV_FD + 1)) "  /* file desc */\n"
                                            "  call  write\n" //通过st_pipe把数据传输给fuzzer
                                            "  addl  $12, %esp\n"
                                            "\n"
                                            "  cmpl  $4, %eax\n"
                                            "  jne   __afl_fork_resume\n" //当失败的时候，可能是因为fuzzer不想使用server
                                            "\n"
                                            ...
                                            "  pushl $" STRINGIFY(FORKSRV_FD) "        /* file desc */\n"
                                                                              ...
                                                                              "\n"
                                                                              "  call fork\n" //否则就调用fork创建test，test用于执行真正的测试用例
                                                                              "\n"
                                                                              "  cmpl $0, %eax\n"
                                                                              "  jl   __afl_die\n"
                                                                              ...
                                                                              "\n"
                                                                              "  movl  %eax, __afl_fork_pid\n" //server把test的pid放在__afl_fork_pid
                                                                              "\n"
                                                                              "  pushl $4              /* length    */\n"
                                                                              "  pushl $__afl_fork_pid /* data      */\n"
                                                                              "  pushl $" STRINGIFY((FORKSRV_FD + 1)) "      /* file desc */\n"
                                                                                                                      "  call  write\n" //server将test的pid告知fuzzer
                                                                                                                      "  addl  $12, %esp\n"
                                                                                                                      "\n"
                                                                                                                      "  pushl $0             /* no flags  */\n"
                                                                                                                      ...
                                                                                                                      "  pushl $__afl_temp /* data      */\n"
                                                                                                                      "  pushl $" STRINGIFY((FORKSRV_FD + 1)) "  /* file desc */\n"
                                                                                                                                                              "  call  write\n" //当test状态改变，waitpid返回，server将收到的test的状态转交给fuzzer
                                                                                                                                                              "  addl  $12, %esp\n"
                                                                                                                                                              "\n"
                                                                                                                                                              "  jmp __afl_fork_wait_loop\n" //再次进入fork_wait_loop状态，可见此状态就是就是准备接收fuzzer指令并产生test的循环
                                                                                                                                                              "\n"
                                                                                                                                                              "__afl_fork_resume:\n"
                                                                                                                                                              "\n"
                                                                                                                                                              ...
                                                                                                                                                              "  pushl $" STRINGIFY(FORKSRV_FD) "\n"
                                                                                                                                                                                                ...
                                                                                                                                                                                                "  pushl $" STRINGIFY((FORKSRV_FD + 1)) "\n"
                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                        "  .comm   __afl_setup_failure, 1, 32\n" //一个标志位，当设置共享内存失败它会被置位
#ifndef COVERAGE_ONLY
                                                                                                                                                                                                                                        "  .comm   __afl_prev_loc, 4, 32\n" //存储上一个位置，即上面R(MAP_SIZE)生成的随机数的值
#endif                                                                                                                                                                                                                                                                      /* !COVERAGE_ONLY */
                                                                                                                                                                                                                                        "  .comm   __afl_fork_pid, 4, 32\n" //存储server fork出来的test的pid
                                                                                                                                                                                                                                        "  .comm   __afl_temp, 4, 32\n"     //一个buffer  4字节
                                                                                                                                                                                                                                        "\n"
                                                                                                                                                                                                                                        ".AFL_SHM_ENV:\n"
                                                                                                                                                                                                                                        "  .asciz \"" SHM_ENV_VAR "\"\n" //常量字串，fuzzer申请的共享内存的shm_id被作为环境变量被赋值给SHM_ENV_VAR存放
                                                                                                                                                                                                                                                                         //server可以通过这个名字取出shm_id以得到共享内存
                                                                                                                                                                                                                                        "\n"
                                                                                                                                                                                                                                        "/* --- END --- */\n"
                                                                                                                                                                                                                                        "\n";
