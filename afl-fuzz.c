...
/* Destructively classify execution counts in a trace. This is used as a
   ...
   must be fast. */
// target是将每个分支的执行次数用1个byte来储存，而fuzzer则进一步把这个执行次数归入以下的buckets中
...
static void cull_queue(void)
{
  ...
  // 根据top_rated设置queue中的favored标志，对queue进行精选，选出favored
  for (i = 0; i < MAP_SIZE; i++)
...
EXP_ST void setup_shm(void)
{
  ...
  // 调用shemget()分配一块共享内存，大小MAP_SIZE为64K
  ...
  // 将共享内存的标志符设置到环境变量中
  // 从而之后fork()得到的子进程可以通过该环境变量，得到这块共享内存的标志符
...
EXP_ST void init_forkserver(char **argv)
{
  ...
  if (pipe(st_pipe) || pipe(ctl_pipe))
    PFATAL("pipe() failed");
  // fuzzer执行fork()得到父进程和子进程
  // 这里的父进程仍然为fuzzer，子进程则为target进程，即将来的fork server
  forksrv_pid = fork();
  ...
  if (!forksrv_pid)
  {
    ...
    // dup2 是重定向函数，就是将文件描述符重定向到函数的第二个参数之后进行返回
    // 这里是将状态和命令两个管道分配到预先指定的fd(文件描述符)
  ...
  // 如果状态管道的信息一切正常，则说明fork server创建完成
  if (rlen == 4)
  {
...
#define FLIP_BIT(_ar, _b)                   \
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++)
  {
    ...
    {
      // 对trace_bits计算hash并来实现fuzzer检查其是否新增了执行路径
...
int main(int argc, char **argv)
{
  ...
  u8 exit_1 = !!getenv("AFL_BENCH_JUST_ONE"); // !! 表示非真就是假
  char **use_argv;
  ...
  // access 检查文件的存在性，如果文件存在，返回0，不存在，返回-1
  doc_path = access(DOC_PATH, F_OK) ? "docs" : DOC_PATH;

  gettimeofday(&tv, &tz); // 获取对应时区的时间
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Q")) > 0)
    ...
    case 'i': /* input dir */
      ...
      in_dir = optarg; // optarg 全局变量，会获取参数后面对应的值
  ...
  {
    hang_tmout = atoi(getenv("AFL_HANG_TMOUT")); // 把参数 str 所指向的字符串转换为一个整数（类型为 int 型）
    if (!hang_tmout)
  ...
  while (1)
  {
    ...
    cull_queue();
    // 判断queue_cur是否为NULL，如果是，则表示已经完成对队列的遍历
    if (!queue_cur)
    {
      // queue_cycle++,初始化相关参数，重新开始遍历队列
      queue_cycle++;
      current_entry = 0;
      cur_skipped_paths = 0;
      queue_cur = queue;
